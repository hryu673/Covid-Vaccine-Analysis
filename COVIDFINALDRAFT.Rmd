...
Based on the posterior median value of $\pi$\footnote{See appendix 8.x for R code.},

$$P(\psi < 0.94855|t) = 0.5$$

we computed $\psi_{median}$ to be 0.94855, which is our point estimate for $\psi$. We computed the Bayesian interval for $\pi$ [a,b], which was [2.2319402%, 8.799074%]\footnote{See appendix 8.x for R code.}, and used it to calculate the interval for $\psi$:

$$P( \frac{1 - 2b}{1 - b} \leq \psi \leq \frac{1 - 2a}{1 - a})$$

Using this, we calculated the Bayesian credible interval for $\psi$ to be [90.35%, 97.6%]\footnote{See appendix 8.x for R code.}. We also determined that the vaccine efficacy rate ($\psi$) exceeds the FDA threshold of 30%. 

$$P(\psi \leq 0.3) = P(\pi \geq \frac{7}{17}) = 1.960014e-28$$
From this, we can state that the probability of $\psi$ being greater than 30% is $1-p$, which is greater than 99%\footnote{See appendix 8.x for R code.}.
...
8.x) Code for Generating Prior and Posterior Distributions of $\pi$
```{r, eval = F}
library(ggplot2)
...
8.x) Code for Calculating Posterior Median of $\pi$
```{r, eval = F}
...
8.x) Code for Calculating Interval Estimate for $\pi$
```{r, eval = F}
int <- qbeta(c(0.025,0.975), shape1 = 8.700102, shape2 = 163)
```

8.x) Code for Calculating Interval Estimate for $\psi$
```{r, eval = F}
upper <- (1 - 2*int[1])/(1-int[1])
lower <- (1 - 2*int[2])/(1-int[2])
```

8.x) Code for Bayesian p-value
```{r, eval = F}
